...
- **Stations in a region, province or municipality:** Use the parameters ``regionId``, ``provinceId``, or ``municipalityId`` to get the stations that correspond to that area. This operation mode also uses the ``limit`` and ``offset`` parameters to generate the pagination.
