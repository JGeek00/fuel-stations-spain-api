...
vi.mock('@/services/migration/migration.service', () => ({
...
vi.mock('@/services/realtime-data/realtime-data.service', () => ({
...
vi.mock('@/services/persisted-data/persisted-data.service', () => ({
...
describe('DatabaseService (PostgreSQL isolated)', () => {
  ...
  it('creates a PostgreSQL connection when validatePostgresDbData returns true', async () => {
    ...
    vi.doMock('@/services/migration/migration.service', () => ({
      migrationService: {
        isDbEmpty: vi.fn().mockResolvedValue(true),
        run: vi.fn().mockResolvedValue(undefined),
      },
    ...
    vi.doMock('@/services/realtime-data/realtime-data.service', () => ({
      realtimeDataService: { loadAll: vi.fn(), registerProgrammedTask: vi.fn() },
    }));
    vi.doMock('@/services/persisted-data/persisted-data.service', () => ({
